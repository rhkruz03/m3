...
API_ID = 21990899
API_HASH = 'e7c4d2dd881abc2cb5b98a83b4201028'
APP_NAME = 'jarafel'
...
async def main(target_id: int, list_only):
    ...
    if not list_only:
        ...
        results = json.dumps(msgs)
        msgs = str(results)
        msgs = msgs.replace (', {"text": null}', '')
        msgs = msgs.replace('": "', ':')
        msgs = msgs.replace('}', '')
        msgs = msgs.replace('{', '')
        msgs1 = msgs.replace('text:', '')
        indexes = [idx for idx, s in enumerate(json.loads(msgs1)) if "EURUSD" in s][0]
        msgs1 = msgs1.replace('"', "'")


        numm = int(indexes)
        lastorder = msgs1.replace("'", '"')
        lastorder = json.loads(lastorder)
        lastorder = lastorder[numm]
        lastorder = lastorder.replace(',',"','")
        lastorder = str("{'" + lastorder + "'}")
        lastorder = lastorder.replace(" ", "")
        lastorder = str(lastorder.replace(":", "':'"))
        lastorder = lastorder.replace("'", '"')
        lastorder = json.loads(lastorder)



        Lotsize = (lastorder["LotSize1"])
        Title = (lastorder["Title"])
        TProfit = (lastorder["TP"])
        SLoss = (lastorder["SL"])
        Entry = (lastorder["Entry"])
        Pips = (lastorder["TotalPips"])

        return Lotsize, Title, TProfit, SLoss, Entry, Pips,
...
with TelegramClient(APP_NAME, API_ID, API_HASH) as client:
    list = client.loop.run_until_complete(main(args.target_id, args.list))
    #print(main(args.target_id, args.list))
...
api_id = 21990899
api_hash = "e7c4d2dd881abc2cb5b98a83b4201028"
phone_number = "19412680356"
chats = ['tvcrossover']
SESSION_NAME = "jarafel"
client = telethon.TelegramClient(SESSION_NAME, api_id, api_hash)
client.session.server_address

client = TelegramClient('jarafel', api_id, api_hash)

client.connect()
if not client.is_user_authorized():
    client.send_code_request(+19412680356)
    me = client.sign_in(phone_number, input('Enter Phone:'))
...
# possibles states for conversation handler6
CALCULATE, TRADE, DECISION = range(3)

# allowed FX symbols
SYMBOLS = ['AUDCHF', 'AUDJPY', 'AUDSGD', 'AUDUSD', 'EURCHF', 'EURUSD', 'EURJPY', 'EURSGD', 'GBPSGD', 'GBPCHF', 'GBPJPY',
           'GBPUSD', 'NOW', 'NZDCHF', 'NZDSGD', 'NZDJPY', 'NZDUSD']
...
def ParseSignal(signal: str) -> dict:
    ...
    if (trade['OrderType'] == 'Buy' or trade['OrderType'] == 'Sell'):
        trade['Entry'] = (signal[1].split())[-2]

    else:
        trade['Entry'] = float((signal[1].split())[-2])

    trade['StopLoss'] = SLoss
    trade['TP'] = TProfit
    ...
    trade['RiskFactor'] = Pips * float((signal[1].split())[-1])
